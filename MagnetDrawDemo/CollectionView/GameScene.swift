...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    //magnet manager
    var manager = MagnetManager.sharedManager() as! MagnetManager
    ...
    override init(size: CGSize) {
        ...
        //setup magnet manager
        manager.setup()
        
        //call movePlayer every magnetic update
        self.manager.onHeadingUpdateListener = { (heading: CLHeading!) -> Void in
            self.movePlayer()
        }
...
//        node.addChild(sprite)
    ...
    func movePlayer(){
        player.position.x = CGFloat(manager.heading!.y)
